...
class EtTapListSlider_AdminPageFramework_Form_View___Script_CollapsibleSection extends EtTapListSlider_AdminPageFramework_Form_View___Script_Base {
...
( function( $ ) {
    ...
    jQuery( document ).ready( function() {
        jQuery( this ).initializeEtTapListSlider_AdminPageFrameworkCollapsibleSections();
    });              
     ...
     * @since    3.7.0
     */
    jQuery( document ).bind( 'et-tap-list-slider_saved_widget', function( event, oWidget ){
        jQuery( oWidget ).initializeEtTapListSlider_AdminPageFrameworkCollapsibleSections();
    });    
    
    $.fn.initializeEtTapListSlider_AdminPageFrameworkCollapsibleSections = function() {
        
        // Expand collapsible sections that are set not to collapse by default 
        jQuery( this ).find( '.et-tap-list-slider-collapsible-sections-title[data-is_collapsed=\"0\"]' )
            .next( '.et-tap-list-slider-collapsible-sections-content' )
            .slideDown( 'fast' );
        jQuery( this ).find( '.et-tap-list-slider-collapsible-section-title[data-is_collapsed=\"0\"]' )
            .closest( '.et-tap-list-slider-section-table' )
            .find( 'tbody' )
            .slideDown( 'fast' );
            
        // Hide collapsible sections of 'section' containers as they are somehow do not get collapsed by default.
        jQuery( this ).find( '.et-tap-list-slider-collapsible-section-title[data-is_collapsed=\"1\"]' )
            .closest( '.et-tap-list-slider-section-table' )
            .find( 'tbody' )
            .hide();
        
        // Bind the click event to the title element.
        jQuery( this ).find( '.et-tap-list-slider-collapsible-sections-title, .et-tap-list-slider-collapsible-section-title' )
            .enableEtTapListSlider_AdminPageFrameworkCollapsibleButton();
        
        // Insert the toggle all button.
        jQuery( this ).find( '.et-tap-list-slider-collapsible-title[data-toggle_all_button!=\"0\"]' ).each( function(){
            
            var _oThis        = jQuery( this ); // to access from inside the below each() method.
            var _bForSections = jQuery( this ).hasClass( 'et-tap-list-slider-collapsible-sections-title' );   // or for the 'section' container.
            ...
            jQuery.each( _aPositions, function( iIndex, _sPosition ) {
                ...
                var _sLeftOrRight = -1 !== jQuery.inArray( _sPosition, [ 'top-right', 'bottom-right', '0' ] )   // if found
                    ? 'right'   // default
                    : 'left';            
                _oButton.find( '.et-tap-list-slider-collapsible-toggle-all-button' ).css( 'float', _sLeftOrRight );
                ...
                if ( _bForSections ) {
                    var _oTargetElement = 'before' === _sTopOrBottom
                        ? _oThis
                        : _oThis.next( '.et-tap-list-slider-collapsible-content' );
                        _oTargetElement[ _sTopOrBottom ]( _oButton );
                } else {    // for 'section' containers
                    _oThis.closest( '.et-tap-list-slider-section' )[ _sTopOrBottom ]( _oButton );
                }                
                ...
                _oButton.click( function(){                  
                    ...
                    var _oButtons = _bForSections
                        ? jQuery( this ).closest( '.et-tap-list-slider-sectionset' ).siblings().andSelf().find( '> .et-tap-list-slider-collapsible-toggle-all-button-container' )
                        : jQuery( this ).siblings( '.et-tap-list-slider-collapsible-toggle-all-button-container' ).andSelf();
                    _oButtons.toggleClass( 'flipped' );
                    if ( _bForSections ) {
                        _oButton.parent().parent().children().children( '* > .et-tap-list-slider-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    } else {
                        _oButton.closest( '.et-tap-list-slider-sections' ).children( '.et-tap-list-slider-section' ).children( '.et-tap-list-slider-section-table' ).children( 'caption' ).children( '.et-tap-list-slider-collapsible-title' ).each( function() {
                            jQuery( this ).trigger( 'click', [ 'by_toggle_all_button' ] );
                        } );
                    }
     ...
     */
    $.fn.enableEtTapListSlider_AdminPageFrameworkCollapsibleButton = function() {
        ...
        function _isFieldElement( nodeTarget ) {
            ...
            if ( jQuery( nodeTarget ).hasClass( 'et-tap-list-slider-collapsible-button' ) ) {
                return false;
            }
        ...
        jQuery( this ).click( function( event, sContext ){
            ...
            var _sContainerType = jQuery( this ).hasClass( 'et-tap-list-slider-collapsible-sections-title' )
                ? 'sections'
                : 'section';
            var _oTargetContent = 'sections' === _sContainerType
                ? jQuery( this ).next( '.et-tap-list-slider-collapsible-content' ).first()
            ...
            if ( 'expand' === _sAction && _oThis.data( 'collapse_others_on_expand' ) ) {
                _oThis.parent().parent().children().children( '* > .et-tap-list-slider-collapsible-content' ).not( _oTargetContent ).slideUp( 'fast', function() {
                    jQuery( this ).prev( '.et-tap-list-slider-collapsible-title' ).addClass( 'collapsed' );
                });
            }
...
JAVASCRIPTS;
    ...
    static private function _getToggleAllButtonHTML($oMsg) {
        ...
        return "<div class='et-tap-list-slider-collapsible-toggle-all-button-container'>" . "<span class='et-tap-list-slider-collapsible-toggle-all-button button " . $_sDashIconSort . "'" . " title='" . esc_attr($_sLabelToggleAllSections) . "'>" . $_sText . "</span>" . "</div>";
    }
}
